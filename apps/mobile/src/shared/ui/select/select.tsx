...
/**
 * Wrapper used to host the dropdown above all native content.
 *
 * On iOS the portaled content must live inside a `FullWindowOverlay` so it
 * renders above native modals/navigation; on Android and web that primitive
 * is unnecessary, so a transparent `React.Fragment` is used instead.
 */
const FullWindowOverlay =
	Platform.OS === "ios" ? RNFullWindowOverlay : React.Fragment;

/**
 * Floating panel that holds the select options.
 *
 * Render pipeline: `Portal` (escapes the trigger's layout) →
 * {@link FullWindowOverlay} (iOS stacking) → `Overlay` (full-screen catcher,
 * `absoluteFill` on native) → animated container (fade in/out on native only).
 *
 * Positioning is `"popper"` by default. On web, `props.side` drives the
 * slide/translate animation and the panel is sized to the trigger via CSS
 * variables; on native it falls back to simple padding. Pass `portalHost` to
 * target a specific `PortalHost`.
 */
